...
# Some documents put the section number at the END of the heading rather than
# the start:
#
#     "Part 1 and Part 2 5.1.1.8"      not   "5.1.1.8 Part 1 and Part 2"
#     "Safety Endpoints 10.4.2"        not   "10.4.2 Safety Endpoints"
#
# Measured on a 180-page oncology protocol with no PDF outline: 51% of its 220
# headings carry a trailing dotted number, against 9% with a leading one. With
# only the leading pattern, that document fell below MIN_NUMBERED_SHARE, the
# renumbering pass skipped it entirely, and two deep subsections ended up as
# document roots — `Part 1 and Part 2 5.1.1.8` swallowing 154 chunks and
# `Safety Endpoints 10.4.2` swallowing 90.
#
# Requires at least one dot. A bare trailing integer is far more often a page
# number, a year, a dose or a count ("Cohort 2", "Grade 3") than a section
# reference, and matching those would be worse than matching nothing.
TRAILING_SECTION = re.compile(r"\s(\d+(?:\.\d+)+)\s*$")
...
def _numbering(text: str) -> str | None:
    """The section number in a heading, leading or trailing, or None.

    Leading is checked first: a heading carrying both ("5.2 Dose 10.4.2") is
    far more likely to be section 5.2 mentioning 10.4.2 than the reverse.
    """
    stripped = text.strip()
    match = DOTTED_SECTION.match(stripped)
    if match:
        return match.group(1)
    match = TRAILING_SECTION.search(stripped)
    return match.group(1) if match else None


def _numbered_level(text: str) -> int | None:
    """Depth implied by a heading's own numbering, or None if unnumbered."""
    number = _numbering(text)
    return number.count(".") + 1 if number else None


def _section_number(text: str) -> tuple[int, ...] | None:
    """The heading's section number as a tuple of ints, or None."""
    number = _numbering(text)
    if number is None:
        return None
    try:
...
# How far a top-level section number may jump forward and still be believed.
#
# A perfect sequence advances by one, but real documents skip: the layout model
# misses a heading, or a section has no chunk of its own. Measured on a 180-page
# oncology protocol, the genuine top-level sequence was
#
#     1, 3, 4, 6, 9, 11        forward jumps of +2 and +3
#
# while the intruders were `12. Vascular disorders` (+6, a list item inside an
# adverse-events table) and an appendix restarting at 2 (-9).
#
# So a forward jump of a few is normal and a BACKWARD jump is the reliable
# signal — a section number never decreases within one numbering sequence.
# Backward is always rejected regardless of size; forward is allowed up to this.
#
# The cost of this tolerance: after a legitimate restart (an appendix numbering
# its own sections from 1), everything following is nested rather than promoted,
# because it never regains the sequence. Better than a subsection swallowing
# 154 chunks as a document root, which is what happens with no check at all.
MAX_SECTION_JUMP = 3


def _continues_sequence(number: tuple[int, ...], last_top: int | None) -> bool:
    """Is a single-component number plausibly the next top-level section?

    THE PROBLEM THIS SOLVES

    `21. Section 6.3.3 (Administration):` and `9 References` are textually
    identical in shape — a number, then a title. Depth alone cannot tell them
    apart, and reading `21.` as a level-1 section makes it REPLACE the section
    that actually contains it.

    Measured on a 112-page protocol, the real sequence and the intruders:

        8     8 Appendices
        8.1   8.1 Protocol Version History
        21    21. Section 6.3.3 (Administration):     <- jumps 13
        26    26. Administrative:                     <- amendment change-log
        8.2   8.2 Participating Study Sites           <- real sequence resumes
        ...
        8.5   8.5 RECIST 1.1 and irRECIST Guidelines
        1     1. For Subjects with Measurable Disease <- goes BACKWARD
        2     2. For Subjects with Non-Measurable
        8.6   8.6 Exploratory Assessment              <- resumes again

    Both intruder runs are obvious in sequence and invisible in isolation: a
    section sequence advances by one, never jumps to 21, and never restarts at
    1 in the middle of section 8.

    Multi-component numbers (8.2, 4.3.1) are exempt — a dotted number states
    its own parent, so it cannot be confused for a top-level section.
    """
    if len(number) > 1:
        return True            # dotted numbers are self-describing
    if last_top is None:
        return True            # the first numbered heading has nothing to follow
    jump = number[0] - last_top
    # Backward is never a section sequence; forward is allowed a small gap for
    # headings the layout model missed.
    return 0 < jump <= MAX_SECTION_JUMP
...
def renumber_from_text(doc, verbose: bool = True) -> int:
    ...
    changed = 0
    last_numbered = 0
    last_top = None          # the last believed top-level section number
    rejected = []            # numbers that looked like sections but broke sequence
    ...
    for item, level in zip(headings, levels):
        ...
        elif not _continues_sequence(number, last_top):
            # Numbered, but the number does not continue the section sequence
            # — a list item, not a section. Nest it like an unnumbered heading
            # rather than letting it replace the section that contains it.
            rejected.append(text.strip()[:40])
            new_level = last_numbered + 1 if last_numbered else 1
        else:
            new_level = level
            last_numbered = level
    ...
    if verbose:
        ...
        if rejected:
            print(f"    {len(rejected)} numbered heading(s) did not continue the "
                  f"section sequence and were nested instead of promoted: "
                  f"{', '.join(repr(r) for r in rejected[:4])}"
                  + (f" (+{len(rejected) - 4} more)" if len(rejected) > 4 else ""),
                  flush=True)
    return changed
